...
def test_multi_roundtrip_after_promote(multi_bundle):
    ...
    from knowledge_graph.builders.sv.unlift import unlift, emit  # noqa: PLC0415
...
def test_cone_of_influence_crosses_hierarchy(multi_bundle):
    ...
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import cone_of_influence, find_by_name
...
def test_tool_instances_of(multi_bundle):
    ...
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import instances_of
...
def test_tool_sensitivity_of(multi_bundle):
    ...
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import queryable_nodes, sensitivity_of
...
def test_s12_generate_for_elaborated_instances(multi_bundle):
    ...
    * Each block instantiates u_fifo_gen which has an of_module edge to fifo.
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import find_by_name, instances_of, neighbors
...
def test_s11_modports_of_interface(multi_bundle):
    ...
    * It has three modports: producer, consumer, dut, each with per-signal
    ...
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import find_by_name, neighbors, modports_of
...
def test_s10_function_calls_and_cone(multi_bundle):
    ...
    * cone_of_influence('fifo.wr_ptr') still bridges through the function call
    ...
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import find_by_name, neighbors, cone_of_influence
...
def test_s9_package_of_typedef(multi_bundle):
    ...
    * EMPTY/NORMAL/FULL are enum_value declarators under fifo_status_e
    ...
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import find_by_name, neighbors, package_of
...
def test_s8_always_comb_cone_of_status(multi_bundle):
    ...
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import cone_of_influence, queryable_nodes, sensitivity_of
...
def test_s7_param_overrides_per_instance(multi_bundle):
    ...
    """
    _tree, _comp, graph = multi_bundle
    from knowledge_graph.builders.sv.semantic import param_overrides
