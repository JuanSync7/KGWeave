...
class GraphStorageBackend(ABC):
    ...
    # ------------------------------------------------------------------
    # KGAdminClient surface — concrete defaults (Step 4)
    # ------------------------------------------------------------------

    def count_by_source_key(self, source_key: str) -> int:
        """Return the number of entities currently tagged with *source_key*.

        Default implementation walks ``get_all_entities()``. Backends that
        maintain a per-source index may override for efficiency.
        """
        return sum(
            1 for entity in self.get_all_entities() if source_key in entity.sources
        )

    def delete_by_source_key(self, source_key: str) -> RemovalStats:
        """Alias for :meth:`remove_by_source` — KGAdminClient Protocol name."""
        return self.remove_by_source(source_key)

    def health(self) -> Dict[str, object]:
        """Return diagnostic stats plus a ``backend`` identifier.

        Default uses ``stats()`` and tags the dict with the lowercase backend
        class name (stripped of a trailing ``Backend`` suffix).
        """
        info: Dict[str, object] = dict(self.stats())
        cls_name = type(self).__name__
        if cls_name.endswith("Backend"):
            cls_name = cls_name[: -len("Backend")]
        info.setdefault("backend", cls_name.lower())
        return info
